...
import org.apache.spark.storage.{RDDBlockId, BlockManager, StorageLevel}
...
class CacheManagerSuite extends FunSuite with BeforeAndAfter with EasyMockSugar {
  ...
  test("get uncached rdd") {
    expecting {
      blockManager.get(new RDDBlockId(0, 0)).andReturn(None)
      blockManager.put(new RDDBlockId(0, 0), ArrayBuffer[Any](1, 2, 3, 4), StorageLevel.MEMORY_ONLY,
        true).andReturn(0)
    }
  ...
  test("get cached rdd") {
    expecting {
      blockManager.get(new RDDBlockId(0, 0)).andReturn(Some(ArrayBuffer(5, 6, 7).iterator))
    }
