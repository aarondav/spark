...
import org.apache.spark.network.shuffle.{OneForOneBlockFetcher, BlockFetchingListener, ShuffleMessages}
...
import ShuffleMessages.{RegisterExecutor}
...
class NettyBlockTransferService(conf: SparkConf) extends BlockTransferService {
  ...
  override def fetchBlocks(
    ...
    logInfo(s"Fetch blocks from $host:$port (exec $execId)")
    try {
